<template>
  <v-row>
    <v-col cols="12" md="4">
      <div class="border">
        <p class="text-center text-subtitle-1 mt-3 font-weight-medium">
          Messages
        </p>
        <v-text-field
          label="Search"
        ...
        <!-- chat items -->
        <div class="d-flex">
          <v-virtual-scroll :items="chatStore.chats" item-height="50">
              ...
              >
                ...
                <template #prepend>
                  <v-badge
                    ...
                    bordered
                  >
                    <v-avatar :image="profileImage(chat.client, '')" />
                  </v-badge>
                </template>
                <template #append>
    ...
    <v-col cols="12" md="8">
      <div v-if="selectedChat" class="border h-100">
        ...
        >
          ...
          <template v-for="message in selectedChat.messages" :key="message.id">
            <div
              ...
              :class="{
                'justify-end': message.sender === selectedChat.freelancer,
              }"
            >
              <v-card
                flat
                class="mt-1 w-75"
                :prepend-avatar="profileImage(selectedChat.client, '')"
                :variant="
              ...
              >
                <template #prepend>
                  <v-avatar
                    v-if="message.sender === selectedChat.freelancer"
        ...
        <div class="d-flex ga-2 mx-3 mt-6">
          <v-textarea
...
const page = ref(1);
onMounted(async () => {
  await chatStore.fetchChats({ page: page.value });
});
